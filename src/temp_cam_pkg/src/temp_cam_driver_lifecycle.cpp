...
public:
    ThermalCameraNode(const std::string &port_name)
        : LifecycleNode("temp_cam_driver_lifecycle"),
    ...
    {
        // 如果上一次 io_thread_ 没有正确释放，先 join
        if (io_thread_.joinable()) {
            io_thread_.join();
        }
        // 如果 io_context 已经 stop，需要重建
    ...
    CallbackReturn on_deactivate(const State &)
    {
        try {
            ...
            RCLCPP_ERROR(this->get_logger(), "Exception in deactivate: %s", e.what());
            return CallbackReturn::FAILURE;
        }
        return CallbackReturn::SUCCESS;
    }
    ...
    CallbackReturn on_cleanup(const State &)
    {
        try {
            ...
            // 重新初始化 io_context_ 和 serial_port_
            io_context_.restart();
            serial_port_ = boost::asio::serial_port(io_context_);
